...
public class KitCommand {
    ...
    @Command(name = {"kit"}, playerOnly = true)
    ...
    public void createKit(CommandArgs args) {
        ...
        if (args.length() == 0) {
            player.sendMessage(CC.RED + "You have to give the kit a name!");
            return;
        }
        ...
        Kit kit1 = new Kit(name, Arrays.stream(player.getInventory().getContents())
