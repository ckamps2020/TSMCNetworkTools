...
public final class AdminChatCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (sender instanceof Player) {
			...
			if (main.hasPerm(user, "tools.staff.adminchat")) {
				if (args.length == 0) {
					...
					if (args[0].equalsIgnoreCase("global")) {
						tempData.setAdminchatSetting(MessageSettings.GLOBAL);
						player.sendMessage(StringUtils.msg("&e&lADMIN CHAT &6■ &7You toggled Admin Chat settings to &eglobal&7!"));
						return true;
					} else if (args[0].equalsIgnoreCase("server")) {
						tempData.setAdminchatSetting(MessageSettings.LOCAL);
						player.sendMessage(StringUtils.msg("&e&lADMIN CHAT &6■ &7You toggled Admin Chat settings to &eserver&7!"));
						return true;
					}
					...
					String finalMessage = "&8[&c&lAC&8] " + metaData.getPrefix() + " " + player.getName() + " &8» &e" + stringBuilder.toString();
					if (tempData.getAdminchatSetting() == MessageSettings.GLOBAL) {
						...
						for (Player p : Bukkit.getOnlinePlayers()) {
							...
							if (main.hasPerm(u, "tools.staff.adminchat")) {
								if (data.isAdminchatEnabled() && data.getAdminchatSetting() == MessageSettings.LOCAL) {
