    ...
    public void handle(String channel, JsonObject object) {
        switch (channel) {
            case RedisChannels.MESSAGE: {
                ...
                boolean bypass = object.get("bypass").getAsBoolean();
                ...
                if (!bypass && targetUser.getIgnoredPlayers().contains(sender)) {
                    response.set("type", MessageResponse.IGNORED.name());

                } else if (!bypass && !targetUser.getSetting(PlayerSetting.PRIVATE_MESSAGES)) {
